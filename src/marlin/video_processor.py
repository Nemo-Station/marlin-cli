...
Pure chunking library — no model loading, no file I/O, no CLI.
Splits a video into overlapping chunks, calls a user-supplied ground_fn
on each, normalises local timestamps to global, deduplicates overlap
events, and returns the combined result in memory.

Usage from cli.py::

    from .chunk import probe_duration_seconds, find_in_long_video
...
def extract_chunk(
    ...
    """Extract a single chunk from the source video via FFmpeg (stream-copy)."""
    ...
    if proc.returncode != 0:
        raise FFmpegError(
            f"FFmpeg failed for chunk {chunk.chunk_id}: {proc.stderr.strip()}"
        )
...
def dedup_hits(
    ...
    When two hits start within *tolerance* seconds of each other, keep
    the one with the longer span (tie-break: later chunk).
    """
    if not hits:
        return []
    ...
    for candidate in sorted_hits[1:]:
        prev = kept[-1]
        if abs(candidate.global_start - prev.global_start) <= tolerance:
...
def find_in_long_video(
    ...
    logger.info(
        ...
        video_path.name, duration, total, chunk_seconds, overlap_seconds,
    )
    ...
    try:
        for chunk in chunks:
            ...
            try:
                ...
                logger.warning("ground_fn failed on chunk {}: {}", chunk.chunk_id, exc)
                continue

            if tier == "no_match" or local_end <= local_start:
                continue
