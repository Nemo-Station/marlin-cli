    ...
    *,
    ...
    if duration is None:
        try:
            ...
            for e in events:
                if hasattr(e, "end"):
                    max_end = max(max_end, getattr(e, "end"))
    ...
    for e in events:
        if hasattr(e, "start") and hasattr(e, "end") and hasattr(e, "text"):
            normalized_events.append({
                "global_start": getattr(e, "start"),
                "global_end": getattr(e, "end"),
                "description": getattr(e, "text"),
    ...
    patched_html = html_template.replace(
        "__DATA_JSON_PLACEHOLDER__", json.dumps(data, indent=2)
    ).replace(
    ...
    if not is_json():
        console.print(
            f"  [ok]✓[/ok] visualizer → [bold]{output_html_path.name}[/bold]"
        )
